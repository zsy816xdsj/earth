...
abstract class animal extends organism{
  ...
  animal(float s,float x,float y){
    ...
    Width = s;
    Height = s;
    energy = int(random(1200,1350));//change this to give more energy
    life = int(random(700,900));//change this to give more life
    thirthty = int(random(1200,1350));//change this to give more thirthty
 ...
 void drinking(earth target){
   if(dist(pos.x,pos.y,target.pos.x,target.pos.y) <= 20){
     thirthty = thirthty+200;//change this to give more energy each time
     onTheWayToDrink = false;
   }
 }
 ...
 void eating(earth target){
   if(dist(pos.x,pos.y,target.pos.x,target.pos.y) <= 20){
     energy = energy+100;//change this to give more energy each time
     onTheWayToFood = false;
     target.be_eaten();
   }
 }
    ...
    if(life <= 0){
        ...
        if(energy <= 0){
          ...
          if(thirthty <= 0){
            ...
            if(energy<=1200&&h_animal.size()>0){
              if(onTheWayToFood){
                moveto(food);
                eating(food);
              }else{
            ...
            if(thirthty<=1200&&lake.size()>0&&!onTheWayToFood){
              if(onTheWayToDrink){
                moveto(water);
                drinking(water);
              }else{
